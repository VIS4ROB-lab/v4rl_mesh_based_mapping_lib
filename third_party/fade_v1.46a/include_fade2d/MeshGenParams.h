 ...
 * aligned along this vector. By default %s gridVector is axis aligned.
 */
	Vector2 gridVector;

/** \brief gridLength
 *
 * Set %gridLength > 0 to insert grid points before the mesh generator
 * runs. By default % gridLength=0 (off).
 *
 * @warning Small values can produce a huge number of triangles
 */
	double gridLength;

/** \brief Commad
 *
 * A command string, not for public use.
 */
//	std::string command;
};
